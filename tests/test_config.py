...
def test_defaults_resolve_under_project_root():
    for var in ("SKALD_DATA_DIR", "SKALD_CONFIG_DIR", "SKALD_OUTPUT_DIR"):
        os.environ.pop(var, None)
    config = _reload_config()

    assert config.DATA_DIR == os.path.join(config.PROJECT_ROOT, "data")
    assert config.CONFIG_DIR == os.path.join(config.BASE_DIR, "config")
    assert config.OUTPUT_DIR == os.path.join(config.PROJECT_ROOT, "output")
