"""
verify.py — Stage 6: Verification + escalation. Re-runs PaddleOCR on targeted redacted
image regions to confirm zero residual PHI text, escalating redaction if needed.
"""
...
def verify_redaction(cleaned_array, phi_regions, paddle_ocr=None, analyzer=None):
    """Re-runs targeted PaddleOCR on redacted ROI regions to confirm zero residual PHI text."""
    log.info("  [Stage 6] Verification pass — running fast targeted PaddleOCR on redacted ROIs...")
    ...
    for region in residual:
        ...
        escalated = redact_roi(escalated, x1e, y1e, x2e, y2e, pad=8)
        escalation_count += 1
